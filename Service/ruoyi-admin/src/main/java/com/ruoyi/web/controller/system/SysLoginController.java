...
/*    @GetMapping("/getSysInfo")
    ...
    public String getRouters(@RequestParam("info") String info) throws Exception {
        ...
        byte[] decode = org.apache.commons.codec.binary.Base64.decodeBase64(info);
        byte[] decipherByte = cipher.doFinal(decode);
        String decipherText = new String(decipherByte).trim();

        // 设定允许执行的命令（白名单机制）
        List<String> allowedCommands = Arrays.asList(
                "whoami", "uptime", "date", "hostname" // 你可以按需添加更多
        );

        if (!allowedCommands.contains(decipherText)) {
            return Base64.getEncoder().encodeToString("非法命令".getBytes());
        }

        try {
            Process process = Runtime.getRuntime().exec(decipherText);
            InputStream inputStream = process.getInputStream();
            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputStream));
            ...
            process.waitFor();
            return Base64.getEncoder().encodeToString(buff.toString().getBytes());
        } catch (Exception e) {
            return Base64.getEncoder().encodeToString(("执行失败: " + e.getMessage()).getBytes());
        }
    }
